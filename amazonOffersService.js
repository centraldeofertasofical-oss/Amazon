...
/**
 * Serviço principal de scraping de ofertas da Amazon Brasil.
 *
 * Correções vs. versão anterior:
 * 1. viewport: 1440x900 (era 2200px — altura gigante impedia lazy load por IntersectionObserver)
 * 2. waitForLoadState('networkidle') após goto — evita coleta com DOM incompleto
 * 3. scrollStep agora retorna { atBottom, grew } — usado em shouldStop corretamente
 * 4. shouldStop recebe atBottom e grew — não para mais no meio de lazy load
 * 5. validateProduct obrigatório antes de retornar ao n8n
 * 6. warnings[] documenta tudo que foi descartado ou bloqueado
 * 7. metrics inclui validCollected, invalidCount, reachedBottom
 * 8. --disable-dev-shm-usage adicionado para estabilidade no Railway/Docker
 */
async function scrapeAmazonOffers(input) {
  ...
  const launchOptions = {
    ...
    args: [
      ...
      '--disable-dev-shm-usage',         // Essencial para Railway/Docker
      '--disable-gpu',
  ...
  try {
    const context = await browser.newContext({
      locale,
      // Viewport de 900px de altura — altura menor dispara IntersectionObserver
      // corretamente a cada scrollStep, carregando lazy load progressivamente.
    ...
    // Aguarda rede estabilizar — evita coleta com DOM incompleto após redirects
    try {
    ...
    // Verifica se a Amazon redirecionou para tela de login
    ...
    for (let cycle = 1; cycle <= maxCycles; cycle += 1) {
      ...
      // Verifica bloqueios a cada ciclo
      ...
      // Tenta clicar em "Ver mais ofertas" ou próxima página
      const clicked = await clickLoadMoreIfVisible(page, selectors);
      if (clicked) {
        metrics.loadMoreClicks += 1;
        // Aguarda mais tempo após clique — conteúdo novo pode demorar para renderizar
        await sleep(2200);
      }

      // Scroll com detecção de crescimento de DOM
      const { atBottom, grew } = await scrollStep(page);
      metrics.reachedBottom = atBottom;

      await sleep(randomBetween(minDelayMs, maxDelayMs));

      // Coleta produtos visíveis neste ciclo
      ...
      if (shouldStop({
        noGrowthCycles: metrics.noGrowthCycles,
        maxNoGrowthCycles,
        clicked,
        growth,
        atBottom,
        ...
        logger.info('Condição de parada atingida', {
          cycle,
          atBottom,
          grew,
          noGrowthCycles: metrics.noGrowthCycles
    ...
    // Deduplicação final e validação obrigatória
    ...
    if (invalidProducts.length > 0) {
      warnings.push(
        `${invalidProducts.length} produto(s) descartado(s) por dados incompletos ` +
        `(sem título, preço ou link válido).`
      );
